...
signal_ori_load = cell(1,data_num*split_num);
signal_received_load = cell(1,data_num*split_num);
...
for name_order = load_begin:load_end
    ...
    for i_for_load = 1:split_num
        signal_ori_load{split_num*(load_data_loop-1)+i_for_load} = [zeros(1,add_zero),data_ori(split_length*(i_for_load-1)+1 : split_length*i_for_load)];
        signal_ori_load{split_num*(load_data_loop-1)+i_for_load} =  signal_ori_load{split_num*(load_data_loop-1)+i_for_load}/upsample_norm(load_data_loop);
        signal_received_load{split_num*(load_data_loop-1)+i_for_load} = data_received(split_length*rate_times*(i_for_load-1)+1 : split_length*rate_times*i_for_load);
    end
end
signal_ori_load = gather(signal_ori_load);
signal_received_load = gather(signal_received_load);
