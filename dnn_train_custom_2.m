function [ dlnet, velocity, losss, learnRate_save ] = dnn_train_custom_2(rate_times,maxEpochs, xTrain, yTrain, xValidation, yValidation, ...
...
%             [dlnet, velocity] = sgdmupdate(dlnet,gradients,velocity,learnRate,momentum);
            ...
            if mod(ite,floor(numIterPerEpoch/5)) == 0
                fprintf(" training times = %d/%d , epoches = %d , iteration = %d/%d , loss = %e , learnRate = %e \n",...
                    train_time,total_train_time,epoch,ite,numIterPerEpoch,losss(ite),learnRate);
                pause(0.5)
            end
            clear X Y dlX dlY
